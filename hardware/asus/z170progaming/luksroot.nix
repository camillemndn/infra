...
in
{
  ...
  config = mkIf (luks.devices != { } || luks.forceLuksSupportInInitrd) {
    ...
    #        description = "Unlocking LUKS device ${name} using Clevis";
    #        requiredBy = [ "systemd-cryptsetup@${name}.service" "remote-fs-pre.target" "remote-cryptsetup.target" ];
    ...
    #        };
    #        script = ''
    #          ip addr add 192.168.1.3/24 dev enp0s31f6
    #          mkdir -p /clevis-${name}
    #          clevis decrypt < /etc/clevis/${name}.jwe > /clevis-${name}/decrypted
    #        '';
    #      })
    ...
    boot.initrd.systemd.paths."clevis-luks-askpass" = {
    ...
    boot.initrd.systemd.services."clevis-luks-askpass" = {
      ...
      script = ''
        ip addr add 192.168.1.3/24 dev enp0s31f6 || true
        clevis luks unlock -d /dev/disk/by-uuid/90a7cca1-d6ee-48fb-b225-9519ad1e081b 
      '';
    };
