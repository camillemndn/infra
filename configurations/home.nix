...
let
  ...
  mkHmConfigFromUsers =
    ...
    lib.genAttrs (args.users or [ "camille" ]) (
      user: homeManagerConfiguration' { inherit user configuration; }
    );

  mapHmConfigsFromMachines =
    x: lib.flattenAttrs (config: user: user + "@" + config) (lib.mapAttrs mkHmConfigFromUsers x);
in
